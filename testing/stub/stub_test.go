...
// instead of create every stub just to test GetPet in GetPetNames
// we could just write a stub struct that implement only the method we need
...
// the above method could get cumbersome and hard to
// understand when theres a lot of test, (we need to
// implement a struct for each test)
// so we just define a bunch of field value that are
// function call to function that will implement the interface 
// and define the method to make it implement the interface
...
func TestLogicGetPetNames2(t *testing.T) {
	data := []struct {
		...
		// now we can just define rando stub function and
		// return the desired result for each test
