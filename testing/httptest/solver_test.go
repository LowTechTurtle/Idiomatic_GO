...
func TestRemoteSolver_Resolve(t *testing.T) {
	...
	var io info
	// use httptest to create a fake server that
	// would handle the math request correctly
	// the goal is to test if the Resolve function( client
	// will call this code) do the work correctly
	...
	defer server.Close()
	// we get the url of the fake server from server.URL from here
	// also, we can get a client from service.Client()
	rs := RemoteSolver{
