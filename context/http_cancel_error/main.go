...
func main() {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				cancelfunc(fmt.Errorf("error in status goroutines %w", err))
				return
			}

			if resp.StatusCode == http.StatusInternalServerError {
				cancelfunc(fmt.Errorf("error with internal server %w", err))
				return
			}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				cancelfunc(fmt.Errorf("in delay goroutines %w", err))
				return
			}
			select {
